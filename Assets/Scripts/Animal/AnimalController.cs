...
public class AnimalController : MonoBehaviour //调用在每个动物预制体上
{
    ...
    //已成年
    [HideInInspector] public bool isAdult;
    //生产在场景中
    ...
    /// 原地生产动物产品
    /// </summary>
    public void ProduceItemOnTransform(Vector2 playerPos)
    {
        AddAnimalFriendliness(0.25f, playerPos);
        if(Vector2.Distance(transform.position, playerPos) <= touchDistance && isAdult && !isProduce)
        {
            isProduce = true;
            if (animalFriendliness >= 4f)
            {
                int randomAdd = Random.Range(0, 101);
                if (randomAdd + animalFriendliness * 4 > 90)
                {
                    EventHandler.CallInstantiateItemInScene(produceRareItemID, transform.position);
                    return;
                }
            }
